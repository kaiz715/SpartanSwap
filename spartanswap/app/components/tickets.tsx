...
const TicketsPage = () => {
  ...
  const [filters, setFilters] = useState<{ color: string; type: string; price: number[] }>({
    color: "",
    type: "",
    price: [0, 300],
  });
  ...
  return (
    <div className="bg-[#EAF5FA] min-h-screen">
      ...
      <div className="flex justify-between items-center mt-6 px-6">
        <h2 className="text-2xl font-bold text-gray-700">Tickets</h2>
      ...
      <div className="grid grid-cols-4 gap-6 max-w-7xl mx-auto px-6 mt-6">
        ...
        <div className="col-span-3 grid grid-cols-1 mb-20 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-10">        
          ...
          >      
              ...
              />
                {/* Favorite Heart */}
                <motion.button onClick={() => toggleFavorite(product)} className="absolute top-3 right-3 bg-white p-2 rounded-full shadow hover:scale-110" whileTap={{ scale: 0.8 }}>
                  <Heart className={`w-6 h-6 ${favorites.some((fav) => fav.id === product.id) ? "text-red-500 fill-red-500" : "text-gray-400"}`} />
                </motion.button>
              </div>
  ...
  <motion.button onClick={() => setShowFavorites(!showFavorites)} 
