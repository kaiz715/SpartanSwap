...
class User(db.Model):
    ...
    account_create_date = db.Column(db.DateTime, default=datetime.utcnow, server_default=db.func.current_timestamp())
...
class Item(db.Model):
    __tablename__ = 'item'
    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # Consistencia con User
